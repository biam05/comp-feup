...
public class SemanticAnalysisUtils {
    ...
    public static boolean evaluatesToBoolean(GrammarSymbolTable symbolTable, GrammarMethod method, JmmNode node, List<Report> reports) {
        GrammarType type = new GrammarType("Boolean", false);
    ...
    public static boolean evaluatesToInteger(GrammarSymbolTable symbolTable, GrammarMethod method, JmmNode node, List<Report> reports) {

        GrammarType type = new GrammarType("Int", false);
        switch (node.getKind()) {
            case "Call":
                if (type.equals(evaluateCall(symbolTable, method, node, reports))) return true;
            case "Expression":
    ...
    private static boolean evaluateArrayAccess(GrammarSymbolTable symbolTable, GrammarMethod method, JmmNode node, List<Report> reports) {
    ...
    public static boolean evaluateOperationWithBooleans(GrammarSymbolTable symbolTable, GrammarMethod method, JmmNode node, List<Report> reports) {
        List<JmmNode> children = node.getChildren();
    ...
    public static boolean evaluateOperationWithIntegers(GrammarSymbolTable symbolTable, GrammarMethod method, JmmNode node, List<Report> reports) {
        List<JmmNode> children = node.getChildren();
    ...
    public static boolean evaluateNotOperation(GrammarSymbolTable symbolTable, GrammarMethod method, JmmNode node, List<Report> reports) {
        List<JmmNode> children = node.getChildren();
    ...
    private static Report checkTypeIdentifier(boolean isInt, boolean isArray, GrammarType type, String line, String col) {
    ...
    public static GrammarType checkIfIdentifierExists(GrammarSymbolTable symbolTable, GrammarMethod method, String identifier) {
        GrammarType identifierTypeLocal = method.returnTypeIfExists(identifier);
        GrammarType identifierTypeClass = symbolTable.returnFieldTypeIfExists(identifier);
    ...
    public static Report isIdentifier(GrammarSymbolTable symbolTable, GrammarMethod method, JmmNode node, boolean isInt, boolean isArray) {
        String identifier = node.getKind().replaceAll("'", "").replace("Identifier ", "").trim();

        GrammarType typeIdentifier = checkIfIdentifierExists(symbolTable, method, identifier);
    ...
    private static GrammarType evaluateFinalTerms(GrammarSymbolTable symbolTable, GrammarMethod method, JmmNode node, List<Report> reports, boolean rightOperand) {

        List<JmmNode> children = node.getChildren();
        JmmNode firstChild = children.get(0);

        if (firstChild.getKind().contains("Number")) return new GrammarType("Int", false);
            ...
            if (evaluatesToInteger(symbolTable, method, firstChild.getChildren().get(0), reports))
                return new GrammarType("Int", true);
            ...
            return new GrammarType(newIdentifier, false);
        } else if (firstChild.getKind().contains("Identifier")) {
            String identifier = firstChild.getKind().replaceAll("'", "").replace("Identifier ", "");
            GrammarType type = checkIfIdentifierExists(symbolTable, method, identifier);
            if (type == null)
            ...
            return new GrammarType("Boolean", false);
        else if (rightOperand && firstChild.getKind().equals("Expression"))
            return evaluateExpression(symbolTable, method, firstChild, reports, true);

        return null;
    }


    private static boolean evaluateArray(GrammarSymbolTable symbolTable, GrammarMethod method, JmmNode node, List<Report> reports) {

        if (node.getKind().equals("Call")) {
            GrammarType type = evaluateCall(symbolTable, method, node, reports);
            if (type == null) return false;
            if (type.equals(new GrammarType("String", true)) || type.equals(new GrammarType("Int", true))) return true;
        }
    ...
    public static GrammarType evaluateExpression(GrammarSymbolTable symbolTable, GrammarMethod method, JmmNode node, List<Report> reports, boolean rightOperand) {
        List<JmmNode> children = node.getChildren();
        ...
        if (children.size() == 1) {
            ...
            if (child.getKind().equals("And") && rightOperand) {
                ...
                if (evaluateOperationWithIntegers(symbolTable, method, child, reports))
                    return new GrammarType("Int", false);
            } else if (child.getKind().equals("Not") && rightOperand) {
                if (evaluateNotOperation(symbolTable, method, child, reports)) return new GrammarType("Boolean", false);
    ...
    private static GrammarType evaluateCall(GrammarSymbolTable symbolTable, GrammarMethod method, JmmNode node, List<Report> reports) {
        List<JmmNode> children = node.getChildren();
    ...
    private static GrammarType evaluateMethodCall(GrammarSymbolTable symbolTable, GrammarMethod method, List<JmmNode> nodes, List<Report> reports) {
        ...
        if (identifier.getKind().equals("Call")) {
            hasNestedCall = true;
            GrammarType t = evaluateCall(symbolTable, method, identifier, reports);
            if (t != null) {
                GrammarType res = symbolTable.hasImport(t.getName());
                if (t.equals(res)) return t;
                else if (t.getName().equals(symbolTable.getSuper())) return new GrammarType("Accepted", false);
        ...
        if (!hasNestedCall) {
            ...
            if (!identifierKind.equals("This")) {
                ...
                GrammarType res = symbolTable.hasImport(identifierName);
                if (res != null) return res;

                if (!isNew) {
                    GrammarType identifierType = checkIfIdentifierExists(symbolTable, method, identifierName);
                ...
                if (identifierN.equals(symbolTable.getSuper())) return new GrammarType("Accepted", false);
        ...
        for (JmmNode parameter : parameters) {
            GrammarType type = evaluateExpression(symbolTable, method, parameter, reports, true);
        ...
        GrammarType type = symbolTable.getReturnType(methodInfo);

        if (type != null) return type;

        if (identifierN.equals("this") || identifierN.equals(symbolTable.getClassName())) {
            if (!symbolTable.getSuper().equals("")) return new GrammarType("Accepted", false);
        }
    ...
    public static String getTypeParameters(List<JmmNode> parameters) {
        ...
        for (int i = 0; i < parameters.size(); i++) {
            ...
            GrammarType type = new GrammarType(nodeType);
